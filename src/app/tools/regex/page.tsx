        ...
        <div className="flex flex-col gap-3 sm:gap-4">
          <div>
            ...
            <select value={selectedTitle} onChange={handleSelectChange}
              className="w-full px-3 py-2.5 border border-gray-200 rounded-lg bg-gray-50 text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400">
          ...
          <div className="flex flex-col sm:grid sm:grid-cols-3 gap-3 sm:gap-4">
            <div className="sm:col-span-2">
              ...
              <input type="text" value={pattern} onChange={(e) => { setSelectedTitle(""); setPattern(e.target.value); setError(""); }}
                placeholder="예: \\s+" className="w-full px-3 py-2.5 border border-gray-200 rounded-lg bg-white text-gray-800 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400" />
            </div>
            <div>
              ...
              <input type="text" value={flags} onChange={(e) => { setSelectedTitle(""); setFlags(e.target.value); setError(""); }}
                placeholder="g, i, m..." className="w-full px-3 py-2.5 border border-gray-200 rounded-lg bg-white text-gray-800 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400" />
            </div>
          </div>
          ...
          <div>
            ...
            <input type="text" value={replacement} onChange={(e) => { setSelectedTitle(""); setReplacement(e.target.value); setError(""); }}
              placeholder="예:  (공백)  또는 $1" className="w-full px-3 py-2.5 border border-gray-200 rounded-lg bg-white text-gray-800 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400" />
          </div>

          {error && <div className="text-sm text-red-600 bg-red-50 px-3 py-2.5 rounded-lg">{error}</div>}

          <button onClick={applyReplacement}
            className="self-start px-5 sm:px-6 py-2.5 bg-blue-600 text-white text-xs sm:text-sm font-semibold rounded-xl hover:bg-blue-700 active:bg-blue-800 transition-colors">
            🚀 치환 실행
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col md:grid md:grid-cols-2 gap-3 md:gap-4 mb-4">
        <div className="flex flex-col">
          <label className="text-xs sm:text-sm font-semibold text-gray-700 mb-1.5 flex justify-between">
            <span>📝 입력</span>
            <span className="text-xs font-normal text-gray-400">{input.length} 자</span>
          </label>
          <textarea value={input} onChange={(e) => setInput(e.target.value)}
            placeholder="치환할 텍스트를 입력하세요..."
            className="w-full flex-1 min-h-[200px] sm:min-h-[280px] px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-200 rounded-xl bg-white text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 resize-y" />
        </div>
        <div className="flex flex-col">
          <label className="text-xs sm:text-sm font-semibold text-gray-700 mb-1.5 flex justify-between">
            <span>✅ 결과{matchCount > 0 && <span className="ml-2 text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded-full">{matchCount}회</span>}</span>
          </label>
          <textarea value={output} readOnly placeholder="결과가 여기에 표시됩니다..."
            className="w-full flex-1 min-h-[200px] sm:min-h-[280px] px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-200 rounded-xl bg-gray-50 text-gray-800 text-sm focus:outline-none resize-y" />
        </div>
      </div>

      {diffHtml && input !== output && (
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="px-3 sm:px-4 py-2.5 bg-gray-50 border-b border-gray-100 flex justify-between">
            <span className="text-xs sm:text-sm font-semibold text-gray-700">🔍 변경 비교</span>
            <span className="text-xs text-gray-400">
              <span style={{ background: "#fecaca" }} className="inline-block w-2.5 h-2.5 rounded-sm mr-1" />삭제
              <span style={{ background: "#bbf7d0" }} className="inline-block w-2.5 h-2.5 rounded-sm ml-3 mr-1" />추가
            </span>
          </div>
          <div ref={diffRef} className="px-3 sm:px-4 py-2.5 sm:py-3 text-sm text-gray-800 font-mono whitespace-pre-wrap break-all"
