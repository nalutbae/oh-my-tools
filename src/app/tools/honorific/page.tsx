    ...
    <div className="px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <button onClick={() => setLevel("haeyo")}
          className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors ${level === "haeyo" ? "bg-indigo-600 text-white" : "bg-white border border-gray-200 text-gray-700 hover:bg-gray-50"}`}>🙇 해요체</button>
        <button onClick={() => setLevel("hapsyo")}
          className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors ${level === "hapsyo" ? "bg-indigo-600 text-white" : "bg-white border border-gray-200 text-gray-700 hover:bg-gray-50"}`}>🎩 합쇼체</button>
        <button onClick={handleConvert} disabled={!input.trim()}
          className="ml-auto px-3 sm:px-5 py-2 bg-indigo-600 text-white rounded-lg text-xs sm:text-sm font-medium hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">변환하기 →</button>
      </div>
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-2 gap-3 lg:gap-4">
        <div>
          ...
          <textarea ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} onScroll={onInputScroll}
            ...
            className="w-full h-56 sm:h-64 p-3 sm:p-4 border border-gray-200 rounded-xl text-xs sm:text-sm font-medium text-gray-800 bg-white resize-none focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent" />
      ...
      <div className="mt-6 lg:mt-8 p-3 sm:p-4 bg-amber-50 border border-amber-200 rounded-xl">
        ...
        <ul className="text-[10px] sm:text-xs text-amber-700 space-y-1 list-disc list-inside">
