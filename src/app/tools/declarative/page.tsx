...
export default function DeclarativePage() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      const out = await toPlainSpoken(input);
      setOutput(out);
  ...
  useEffect(() => {
    if (!ready) return;
    handleConvert();
  }, [input, ready]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">📝 평서문 변환</h1>
        <p className="text-sm text-gray-500">
          존댓말 텍스트를 평어체(~는다/~다)로 변환합니다 — 대명사(저→나)도 함께 바뀝니다
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div>
          ...
          <textarea
            ...
            className="w-full h-64 p-4 border border-gray-200 rounded-xl text-sm font-medium text-gray-800 bg-white resize-none focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent"
          />
      ...
      <div className="mt-8 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
        <h3 className="text-sm font-semibold text-emerald-800 mb-2">💡 알아두세요</h3>
