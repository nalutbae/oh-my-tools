...
export default function DeclarativePage() {
  ...
  const [customRules, setCustomRules] = useState<DeclarativeRule[]>(DEFAULT_DECLARATIVE_RULES);
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      let result: string;
      if (mode === "simple") {
        result = simpleDeclarative(input, customRules);
        setDiffResult(diffSimpleDeclarative(input, customRules));
      } else {
        result = await toPlainSpoken(input);
  ...
  // 규칙 편집 핸들러
  const updateRule = useCallback((index: number, field: keyof DeclarativeRule, value: string) => {
    setCustomRules((prev) => {
      const next = [...prev];
  ...
  const resetRules = useCallback(() => {
    setCustomRules(DEFAULT_DECLARATIVE_RULES);
  }, []);
          ...
          >
            {diffResult.length === 0 ? (
              ...
              <span className="leading-relaxed whitespace-pre-wrap">
                {diffResult.map((t, i) =>
                  t.changed ? (
                    <mark key={i} className="bg-emerald-200 text-emerald-900 px-0.5 rounded">
                      {t.text}
                    </mark>
                  ) : (
        ...
        <ul className="text-xs text-emerald-700 space-y-1 list-disc list-inside">
          {mode === "simple" ? (
            <>
              ...
              <li>경어체 어미(습니다, 입니다, ㅂ니다 등)를 평서문 어미로 바꾸고, 대명사(저→나, 저희→우리)도 변환합니다.</li>
            ...
            <>
              <li>형태소 분석 모드는 Garu 엔진을 사용하여 더 정교한 변환을 제공합니다.</li>
