    ...
    <div className="px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
      ...
      <div className="flex items-center justify-center gap-2 sm:gap-3 mb-4 sm:mb-5">
        <button onClick={() => setDirection("encode")}
          className={`px-3 sm:px-4 py-2.5 rounded-lg border text-xs sm:text-sm font-semibold transition-colors ${
            direction === "encode" ? "text-blue-600 bg-blue-50 border-blue-200" : "border-gray-200 text-gray-500 hover:bg-gray-50 active:bg-gray-100"}`}>
          🔒 인코딩
        </button>
        <button onClick={swapDirection} className="text-gray-400 hover:text-gray-600 p-1.5 sm:p-2 rounded-lg transition-colors" title="입출력 교환">↔️</button>
        <button onClick={() => setDirection("decode")}
          className={`px-3 sm:px-4 py-2.5 rounded-lg border text-xs sm:text-sm font-semibold transition-colors ${
            direction === "decode" ? "text-green-600 bg-green-50 border-green-200" : "border-gray-200 text-gray-500 hover:bg-gray-50 active:bg-gray-100"}`}>
          🔓 디코딩
        </button>
      </div>
      ...
      <div className="flex flex-col md:grid md:grid-cols-2 gap-3 md:gap-4">
        <div className="flex flex-col">
          ...
          <textarea value={input} onChange={(e) => setInput(e.target.value)}
            ...
            className="w-full flex-1 min-h-[200px] sm:min-h-[260px] px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-200 rounded-xl bg-white text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 resize-y" />
          <button onClick={handleTransform}
            className="mt-2 sm:mt-3 px-4 sm:px-5 py-2.5 bg-blue-600 text-white text-xs sm:text-sm font-semibold rounded-xl hover:bg-blue-700 active:bg-blue-800 transition-colors">
        ...
        <div className="flex flex-col">
          <label className="text-xs sm:text-sm font-semibold text-gray-700 mb-1.5 flex justify-between">
            ...
            <div className="flex items-center gap-1">
              {copied && <span className="text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded">✅ 복사됨</span>}
              <button onClick={copyOutput} disabled={!displayedValue}
                className="text-xs text-gray-400 hover:text-blue-600 px-2 py-1.5 rounded-lg hover:bg-blue-50 active:bg-blue-100 transition-colors disabled:opacity-40">📋 복사</button>
            </div>
          </label>
          <textarea value={displayedValue} readOnly placeholder="결과가 여기에 표시됩니다..."
            className={`w-full flex-1 min-h-[200px] sm:min-h-[260px] px-3 sm:px-4 py-2.5 sm:py-3 border rounded-xl text-sm resize-y ${
          ...
          {!!input && !error && output && (
            <div className="mt-1.5 sm:mt-2 text-xs text-gray-400 text-right">
